...
-- 重置自增序列
ALTER
SEQUENCE sys_menu_id_seq RESTART WITH 1;
...
-- === 顶级菜单 ===
INSERT INTO sys_menu (id, parent_id, name, path, component, icon, sort_order)
VALUES (1, 0, '智能问答', '/rag', 'rag/RagChat', 'ChatDotRound', 1),
       ...
       (5, 0, '系统管理', '/admin', NULL, 'Setting', 10);
-- 父菜单，无组件

-- === 热榜中心 子菜单（parent_id=3） ===
INSERT INTO sys_menu (id, parent_id, name, path, component, icon, sort_order)
VALUES (31, 3, '每日热榜', '/hot/dashboard', 'hot/HotDashboard', 'Sunrise', 1),
       (32, 3, '历史热榜', '/hot/history', 'hot/HotHistory', 'Clock', 2);

-- === 系统管理 子菜单（parent_id=5） ===
INSERT INTO sys_menu (id, parent_id, name, path, component, icon, sort_order)
VALUES (51, 5, '用户管理', '/admin/users', 'admin/UserManage', 'User', 1),
