 ...
 * 爬虫服务代理控制器（仅管理员可用）
 *
 * 将前端请求转发到 local-ai-crawler 服务（:12117），
 * 避免前端直连爬虫服务，统一走 knowledge 的认证和鉴权。
 *
 * 接口前缀：/api/admin/crawler/**
 */
@Slf4j
@RestController
...
public class CrawlerProxyController {
    ...
    private static final ParameterizedTypeReference<Object> OBJECT_TYPE = new ParameterizedTypeReference<>() {};

    public CrawlerProxyController(@Value("${app.crawler.base-url}") String crawlerBaseUrl) {
        this.crawlerClient = RestClient.builder()
    ...
    public Object logsBySource(@PathVariable String source,
