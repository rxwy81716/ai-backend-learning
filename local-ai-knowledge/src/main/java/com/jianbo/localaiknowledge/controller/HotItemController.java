 ...
 * 每日热榜数据查询控制器
 *
 * <p>查询 crawler_hot_item 表中由爬虫采集的热榜数据， 提供每日热搜展示、历史查询、统计分析等 API。
 *
 * <p>接口列表： GET /api/hot/today → 今日全部热榜 GET /api/hot/today/{source} → 今日指定来源热榜 GET
 * /api/hot/date/{date} → 指定日期全部热榜 GET /api/hot/date/{date}/{source} → 指定日期+来源热榜 GET
 * /api/hot/stats/today → 今日各来源采集统计 GET /api/hot/stats/date/{date} → 指定日期统计 GET
 * /api/hot/stats/trend?days=7 → 最近N天趋势 GET /api/hot/top?date=xxx&topN=10 → 各来源 Top N
 *
 * <p>所有接口无需认证（公开数据）。
 */
@Slf4j
...
public class HotItemController {
  ...
  public Map<String, Object> statsToday() {
    ...
    return Map.of("date", today.toString(), "sources", stats);
  }
  ...
  public Map<String, Object> statsByDate(
    ...
    return Map.of("date", date.toString(), "sources", stats);
  }

  /**
   * 最近 N 天的采集趋势（默认7天）
   *
   * <p>返回每天每个来源的采集条目数，适合折线图/柱状图展示。
   */
