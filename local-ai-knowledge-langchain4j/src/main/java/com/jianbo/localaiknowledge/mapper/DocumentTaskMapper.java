...
public interface DocumentTaskMapper {
    ...
    @Update("""
        UPDATE document_task SET status=#{status}, total_chunks=#{totalChunks}, imported_chunks=#{importedChunks},
        error_msg=#{errorMsg}, finished_at=#{finishedAt}, updated_at=NOW() WHERE task_id=#{taskId}
    """)
